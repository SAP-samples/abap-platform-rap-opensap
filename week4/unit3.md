...
## Step 6. Create and implement the behavior implementation (BIL) class for the Booking entity
...
<pre>
rba_travel 				
   	importing	keys_rba 	type table for read import zi_rap_travel_u_1234\\booking\_travel 	
	            	result_requested 	type c length 1  	
	   changing	 result 	type table for read result zi_rap_travel_u_1234\\booking\_travel 	
	            	association_links 	type table for read link zi_rap_travel_u_1234\\booking\_travel 	
              mapped 	type response for mapped early zi_rap_travel_u_1234 	
	     	       failed 	type response for failed early zi_rap_travel_u_1234 
		            reported 	type response for reported early zi_rap_travel_u_1234 	
</pre>

 we see that the import parameter `keys_rba` is basically a table that contains the two key fields `travelid` and `bookingid` with several %control fields that are used to specify which fields shall be read.

<pre>
keys_rba 	type table for read import zi_rap_travel_u_1234\\booking\_travel 
    travelid 	type /dmo/travel_id 
    bookingid 	type /dmo/booking_id 
    %control 
</pre>

In addition there is a flag result_requested which controls whether only key pairs  for the associatoin have to be returned in a table association_links or if the target of the association with all fields shall be returned in the table results.

The legacy API is developed such that information about a Booking entity can only be retrieved by providing the travel_id of the parent entity. 
